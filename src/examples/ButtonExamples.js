  ...
  return (
    <div className={'space-y-8 mb-20 min-h-screen'}>
      ...
      <div className="flex space-x-3 items-center">
        {buttonSize.map((size) => (
          <Button variant={'outlined'} key={`button-size-${size}`} size={size}>
            {size}
          </Button>
        ))}
      </div>
      ...
      <div className="flex space-x-3 items-center">
        {buttonSize.map((size) => (
          <Button key={`button-size-contained-${size}`} variant={'text'} size={size}>
            {size}
          </Button>
        ))}
      </div>
      ...
      <div className="flex gap-3 items-center flex-wrap">
        {buttonColors.map((color) => (
          <Button
            key={`button-size-contained-color-${color}`}
            variant={'contained'}
            size={'normal'}
            color={color}>
            {color}
      ...
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <Button disabled={false} noShadow={true} size={'normal'}>
          Enabled
        </Button>
        <Button disabled={true} size={'large'} variant={'outlined'}>
          Disabled
        </Button>
        <Button disabled={true} size={'normal'} variant={'text'}>
          Disabled
        </Button>
      </div>

      <h3 className="font-bold">Full width</h3>
      <Button color={'deep-orange'} fullWidth={true} rounded={true} size={'large'}>
        fullWidth
      </Button>
    </div>
  );
