...
const SelectCheckboxRadio = () => {
    let checkboxSizes = propSize;
    let checkboxColors = propColors
       ...
       <div className="space-y-6">
        ...
        <div className="space-y-6 mt-12 ">
            ...
            <div className="grid grid-cols-3 gap-6 flex-wrap">
                {checkboxSizes.map(size => <div key={`Radio-${size}`}><Radio  size={size} id={`pilih-radio-${size}`} label={`Pilih ${size}`} /></div>)}
            </div>

            <div className="flex space-x-1 items-center">
                {checkboxSizes.map(size => <div key={`select-Radio-${size}`}><Radio size={size} id={`pilih-radio-me-${size}`} /></div>)}
            </div>

            <div className="flex space-x-1 items-center flex-wrap">
                {checkboxColors.map(color => <div key={`select-Radio-color-${color}`}><Radio size={'normal'} color={color} id={`pilih-radio-me-color-${color}`} label={`Pilih ${color}`}  /></div>)}
            </div>
        </div>
