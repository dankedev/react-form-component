...
const CheckBoxPreview = forwardRef(({ indexKey, type, active, field, ...others }, ref) => {
  const uuid = uuidv4();

  return (
    <PreviewWrap {...others} indexKey={indexKey} active={active} id={uuid}>
        ...
        <div className={`w-full flex-grow`}>
          ...
          <div key={`index-field-name-${field.key}-${indexKey}`} className="col-span-1 space-y-1">
            <div className="px-4 py-1  w-full flex items-center select-none flex items-center space-x-3">
              <span className="w-6 h-6 rounded-md border"></span>
              <span>{field.label}</span>
            </div>
            <div className="px-4 py-1 w-full flex items-center select-none flex items-center space-x-3">
              <span className="w-6 h-6 rounded-md border"></span>
