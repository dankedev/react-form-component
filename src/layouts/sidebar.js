...
const Sidebar = () => {
  ...
  return (
    <div className={'w-[200px] h-screen fixed left-0 top-0 bg-gray-50 border-r py-6'}>
      <div className={'font-bold px-3'}>Pilih component</div>
      <nav className="flex flex-col gap-0">
        {components.map((item) => (
          <Link
            className={'block text-sm px-3 py-2 hover:bg-gray-100'}
        ...
        <Link className={'block text-sm px-3 py-2 hover:bg-gray-100'} to={'/form'}>
          Contoh Form
        </Link>
        <Link className={'block text-sm px-3 py-2 hover:bg-gray-100'} to={'/form-material'}>
          Contoh Form MD
        </Link>
      </nav>
    </div>
