...
import {
    ...
    propTypesClassName,
    propTypesDisabled,
    propTypesChildren, propTypesAutoHeight,
}
    from "./types";
// select components
...
const Select = React.forwardRef(({
    ...
    const containerClasses = classnames(objectsToString(base.container), objectsToString(selectSize.container));
    ...
    const buttonContentClasses = classnames("absolute top-2/4 -translate-y-2/4", variant === "outlined" ? "left-3 pt-0.5" : "left-0 pt-3");
    // 5. set animation
    const animation = {
        unmount: {
            opacity: 0,
    ...
    // 7. select menu
    const selectMenu = (<FloatingFocusManager context={context} preventTabbing>
        <motion.ul {...getFloatingProps({
            ...menuProps,
            ref: floating,
            role: "listbox",
        ...
        })} initial="unmount" exit="unmount" animate={open ? "mount" : "unmount"} variants={appliedAnimation}>
    ...
    // 8. return
    return (<SelectContextProvider value={contextValue}>
        <div ref={ref} className={containerClasses}>
            <button type="button" {...getReferenceProps({
                ...
                {typeof selected === "function" ? (<span className={buttonContentClasses}>
                {selected(children[selectedIndex - 1], selectedIndex - 1)}
              </span>) : (<span className={buttonContentClasses} {...children[selectedIndex - 1]?.props}/>)}
                <div className={arrowClasses}>
                    {arrow ?? (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <NewAnimatePresence>
                {open && (<>
                    {lockScroll ? (<FloatingOverlay lockScroll>{selectMenu}</FloatingOverlay>) : (selectMenu)}
                </>)}
            </NewAnimatePresence>
        </div>
