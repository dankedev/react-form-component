...
import {propTypesValue, propTypesIndex, propTypesDisabled, propTypesClassName, propTypesChildren} from "./types";
...
export const SelectOption = (props) => {
    ...
    const { base } = styles;
    // 2. set @floating-ui
    const { selectedIndex, setSelectedIndex, listRef, setOpen, onChange, activeIndex, setActiveIndex, getItemProps, dataRef, } = useSelect();
    ...
    // 4. return
    return (<li {...rest} role="option" ref={(node) => (listRef.current[index] = node)} className={classes} tabIndex={activeIndex === index ? 0 : 1} aria-selected={activeIndex === index && selectedIndex === index} data-selected={selectedIndex === index} {...getItemProps({
        onClick: (e) => {
